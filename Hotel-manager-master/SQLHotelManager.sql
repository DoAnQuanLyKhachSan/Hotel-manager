...
AS
BEGIN
	SELECT MaPhong,NgayBatDau,TinhTrangThanhToan FROM dbo.PHIEUTHUEPHONG WHERE MaPhong=@MaPhong
END
GO
...
BEGIN
	...
	WHERE ctd.MaPT=pt.MaPT AND pt.MaPT=ctt.MaPT AND pt.MaPhong=@MaPhong
END
GO
...
BEGIN
	SELECT pt.MaPhong,ctt.TenKhachHang,ctd.SoNgayThue,ctd.DonGia,ctd.ThanhTien
	FROM dbo.CHITIET_HOADON ctd, dbo.PHIEUTHUEPHONG pt, dbo.CHITIET_PHIEUTHUE ctt
	WHERE ctd.MaPT=pt.MaPT AND pt.MaPT=ctt.MaPT AND ctt.TenKhachHang= @TenKH
...
BEGIN
	...
	SELECT DISTINCT hd.TongTien FROM dbo.HOADON hd, dbo.CHITIET_HOADON ctd, dbo.PHIEUTHUEPHONG pt
	WHERE hd.MaHD=ctd.MaHD AND ctd.MaPT=pt.MaPT AND pt.MaPhong= @MaPhong
